...
class FileUtils(object):
    ...
    @staticmethod
    def write_text_file(filepath, text, encoding='utf8'):
        try:
            with open(filepath, 'w', encoding='utf8', errors="ignore") as fp:
                fp.write(text)
    ...
    def lock_file(f):
        while True:
            try:
                if platform.system() == PLATFORM_WINDOWS:
                    # todo find solution, cant lock file that is open for writing
                    break
    ...
    @staticmethod
    def saveJSON(path):
        return FileUtils.writeJSON(path)

    @staticmethod
    def readAllAsString(path):
        result = None
        if os.path.exists(path):
            with open(path, 'r') as fp:
                FileUtils.lock_file(fp)
